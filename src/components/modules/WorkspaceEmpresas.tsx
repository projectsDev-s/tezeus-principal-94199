...
import { AdminWorkspaceView } from "@/components/workspace/AdminWorkspaceView";
...
export function WorkspaceEmpresas({ onNavigateToUsers, onNavigateToConfig }: WorkspaceEmpresasProps) {
  ...
  // Determine if user is admin (not master)
  const isAdminOnly = !isMaster && (isAdmin() || userRole === 'admin');
  
  return (
    <div className="p-6">
      {/* Master View: Grid with all workspaces */}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground">
                Gerencie todas as empresas e workspaces
              </p>
            </div>
      ...
      {/* Admin View: Single workspace detailed view */}
      {isAdminOnly && workspaces.length > 0 && (
        <>
          <div className="mb-6">
            <h1 className="text-2xl font-semibold">Minha Empresa</h1>
            <p className="text-muted-foreground">
              Gerencie sua workspace
            </p>
          </div>
          
          <AdminWorkspaceView 
            workspace={workspaces[0]}
            onManageConnections={() => navigate(`/workspace/${workspaces[0].workspace_id}/conexoes`)}
            onManageUsers={() => handleUsersClick(workspaces[0])}
            onManageSettings={() => handleConfigClick(workspaces[0])}
          />
        </>
      )}

      {/* Admin View: No workspace found */}
      {isAdminOnly && workspaces.length === 0 && (
        <div className="text-center py-12">
          <Building2 className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-medium mb-2">Nenhuma empresa atribuída</h3>
          <p className="text-muted-foreground">

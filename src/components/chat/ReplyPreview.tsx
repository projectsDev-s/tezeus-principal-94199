  ...
  const renderMediaPreview = () => {
    ...
    if (messageType === 'image' && message.file_url) {
      return <div className="flex items-center gap-2">
          <img src={message.file_url} className="w-12 h-12 rounded object-cover border border-border flex-shrink-0" alt="Preview" onError={e => {
    ...
    // Texto (fallback)
    return <p className="text-sm text-muted-foreground truncate">
        {message.content.length > 50 ? message.content.substring(0, 50) + '...' : message.content}
      </p>;
  };
  return <div className="flex items-start gap-3 px-4 py-2 bg-muted/50 border-l-4 border-primary">
      ...
      <Button variant="ghost" size="icon" onClick={onCancel} className="h-6 w-6 flex-shrink-0">
        <X className="h-4 w-4" />
      </Button>
    </div>;
}